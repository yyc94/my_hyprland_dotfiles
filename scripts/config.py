...
def _render_config_doc(sources: tuple[Source, ...]) -> str:
    ...
    field_list = lambda fields: ", ".join(f"`{field}`" for field in sorted(fields))
    allowlist = field_list(ALLOWED_EXECUTABLES)
    lines = [
        "<!-- GENERATED FILE. DO NOT EDIT. -->",
        ...
        "# TOML Configuration",
        "",
        "This document is generated from the compiler's registered TOML schemas and describes every field accepted by the compiler. "
        "Generated Lua is deployed under `$HYPRLAND_CONFIG_ROOT/config/generated/`.",
        ...
        "## How to use",
        "",
        "Every domain file below starts with the same required metadata table. Each source is optional as a file; a missing file produces an empty module and never adds hidden defaults. "
        "The keymap has its own schema documented in [KEYMAP.md](KEYMAP.md).",
        "",
        "```toml",
        "[meta]",
        "schema = \"1\"",
        ...
        "```",
        "",
        "`schema` accepts the integer `1` or string `\"1\"`; `target_hyprland` must exactly be `\">=0.55.0\"`; `profile` must be a non-empty string. "
        "The compiler does not infer values from Hyprland defaults. Omit a field to leave that setting untouched.",
        "",
        "## Domains",
        "",
        ...
        "```",
        "",
        "`[commands]` is a table. Every key must match `[A-Za-z_][A-Za-z0-9_]*` so it can be used as a Lua identifier, and every value must be a non-empty string. "
        "The current keymap's five application actions require these exact command keys: `terminal`, `file_manager`, `browser`, `calculator`, and `system_monitor`. "
        "Additional identifier-shaped command keys are allowed for future generated consumers.",
        "",
        "## monitors.toml",
        "",
        "```toml",
        ...
        "```",
        "",
        "The same shape is used for `[[layerrule]]`. `name` is required and unique within its rule kind; `enabled` is optional and defaults to `true`; `match` and `effects` are both required and must each contain at least one field. "
        "The compiler flattens validated effects into the table passed to Hyprland. Matcher strings are passed to Hyprland's regex/workspace/tag engines; regex semantics are not checked by the compiler.",
        "",
        "### Rule entry fields",
        "",
        "| Field | Type and validation |",
        ...
        "| --- | --- | --- |",
        f"| `windowrule` | {field_list(MATCH_STRING_FIELDS)} | non-empty string |",
        f"| `windowrule` | {field_list(MATCH_BOOL_FIELDS)} | boolean |",
        f"| `windowrule` | {field_list(MATCH_INT_FIELDS)} | integer |",
        ...
        "| --- | --- | --- |",
        f"| boolean | {field_list(WINDOW_BOOL_EFFECTS)} | boolean |",
        f"| string | {field_list(WINDOW_STRING_EFFECTS)} | non-empty string |",
        f"| integer | {field_list(WINDOW_INT_EFFECTS)} | `rounding` and `border_size` >= `0`; `rounding` <= `20`; `no_close_for` has no compiler range |",
        f"| number | {field_list(WINDOW_FLOAT_EFFECTS)} | finite number; `scroll_mouse` and `scroll_touchpad` are `0.01` to `10` |",
        f"| vec2 table | {field_list(WINDOW_VECTOR_EFFECTS)} | table with exactly numeric `x` and `y`; compiled as a two-element Lua array |",
        "",
        "### Layer effects",
        "",
        "| Type | Fields | Validation |",
        "| --- | --- | --- |",
        f"| boolean | {field_list(LAYER_BOOL_EFFECTS)} | boolean |",
        f"| string | {field_list(LAYER_STRING_EFFECTS)} | non-empty string |",
        f"| integer | {field_list(LAYER_INT_EFFECTS)} | `above_lock` is `0` to `2`; `order` has no compiler range |",
        f"| number | {field_list(LAYER_FLOAT_EFFECTS)} | `ignore_alpha` is `0` to `1` |",
        "",
